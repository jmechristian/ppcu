...
export default function LearningPage() {
  ...
  useEffect(() => {
    ...
    async function loadLearning() {
      ...
      try {
        setLoading(true);
        setError("");
        const response = await fetch(`/api/thinkific/my-learning?email=${encodeURIComponent(email)}`, {
          method: "GET",
          cache: "no-store",
        });
        ...
        if (!cancelled) {
          setCourses(Array.isArray(json?.data?.courses) ? json.data.courses : []);
        }
      } catch (err) {
        if (!cancelled) {
  ...
  async function handleTakeCourse(course) {
    ...
    const contactName = String(profile.name || `${firstName} ${lastName}`.trim()).trim();
    ...
    <InteriorPageLayout activeItem="learning">
      <section className="min-h-[520px] border border-gray-300 bg-white p-5 sm:p-6">
        <header className="space-y-3 border-b border-gray-200 pb-4">
          <h1 className="text-3xl font-semibold leading-tight text-[#1f2a44]">My Learning</h1>
          <p className="max-w-3xl text-[15px] leading-relaxed text-gray-700">
            View your Thinkific enrollments and progress across your courses.
          </p>
          {takeError && (
        ...
        {!isLoggedIn ? (
          ...
          <div className="mt-6 rounded border border-gray-200 bg-gray-50 px-5 py-6">
            ...
            <p className="mt-2 text-sm text-gray-700">
              You are not enrolled in any Thinkific courses yet. Visit the catalog to enroll and
          ...
          <ul className="mt-6 space-y-3">
            {courses.map((course) => {
              ...
              const safeProgress = progress == null ? 0 : Math.min(Math.max(progress, 0), 100);
              return (
                <li
                  key={course.id}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-col gap-3 sm:flex-row sm:flex-nowrap sm:items-start sm:justify-between sm:gap-4">
                        <div className="min-w-0 sm:flex-1">
                          <h2 className="text-[18px] font-semibold text-gray-900">{course.title}</h2>
                      ...
                      <div className="mt-3">
                        <div className="mb-1 flex items-center justify-between text-xs text-gray-600">
                          ...
                          <span>{progress == null ? "--" : `${safeProgress}%`}</span>
                        </div>
